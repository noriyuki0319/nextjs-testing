...
describe('Todos page / useSWR', () => {
  ...
  it('Should render CSF data after pre-rendered data', async () => {
    ...
    expect(await screen.findByText('3: Static task C')).toBeInTheDocument()
    expect(screen.getByText('4: Static task D')).toBeInTheDocument()
    expect(await screen.findByText('1: Static task A')).toBeInTheDocument()
    expect(screen.getByText('2: Static task B')).toBeInTheDocument()
  })
